...
def main():
    ...
    parser.add_argument("--rtems_version", type=str, help="RTEMS Version")
    ...
    parser.add_argument("-v", "--verbose", help="Verbose CMake build configuration",
                        action="store_true")
    parser.add_argument("-f", "--force", help="Force deletion of existing build folder",
                        action="store_true")
    ...
    if args.rtems_version is not None:
        rtems_version_arg = f"-DRTEMS_VERSION=\"{args.rtems_version}\""
    else:
        rtems_version_arg = ""
    ...
    cmake_command = f"cmake {generator_cmake_arg} -DCMAKE_BUILD_TYPE=\"{cmake_build_type}\" " \
                    f"{cmake_rtems_bsp} {rtems_prefix_arg} {rtems_version_arg} {define_string} {rtems_verbose_args} "\
                    f"{source_location}"
